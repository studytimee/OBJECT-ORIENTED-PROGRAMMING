			...
			do {
				...
				if (command.equals("createuser")) {
					...
					if (check == 1)// login successfull
					{
						...
						if (new File(FileManager.dir_userAcc+username+".txt").exists()) {
							list(out);
						} else

							new File(FileManager.dir_userAcc+username+".txt").createNewFile();

						if (new File(FileManager.dir_userAcc+username).exists()) {
							list(out);
						} else
